...
public class LoginForStudentInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (flag) {//对要拦截的路径进行判断（当前不需要）
           ...
           if(studentObject==null){
               ...
               Student student=studentService.get(((Student)studentObject).getStudentNumber());
               session.setAttribute("student",student);//更新Admin的值
               flag=true;
           }
        }
        return flag;
