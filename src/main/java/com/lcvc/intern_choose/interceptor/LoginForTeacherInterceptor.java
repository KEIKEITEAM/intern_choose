package com.lcvc.intern_choose.interceptor;
...
public class LoginForTeacherInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (flag) {//对要拦截的路径进行判断（当前不需要）
            ...
            //对账户是否登陆进行验证
            //返回错误信息
            Object teacherObject=session.getAttribute("teacher");
           if(teacherObject==null){
               ...
               Teacher teacher=teacherService.get(((Teacher)teacherObject).getTeacherNumber());
               session.setAttribute("teacher",teacher);//更新Admin的值
               flag=true;
           }
        }
        return flag;
